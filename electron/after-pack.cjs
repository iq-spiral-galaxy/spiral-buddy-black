...
//
// v0.5.95 — dock 호버 이름을 "Spiral Buddy Black"로 띄우기 위한 핵심 조각.
//
// 배경 (실측으로 확정):
...
//   "Spiral Buddy Black"로 두면 "Spiral Buddy Black Helper.app"을 찾는데,
//   electron-builder는 Helper를 productName 기준 "Spiral Buddy Helper.app"로
//   패키징 → 이름 불일치 → "Unable to find helper app" FATAL(크래시).
//
// 해법: extendInfo로 CFBundleName="Spiral Buddy Black"를 설정하고, 이 훅에서
// Helper 앱 4종(+executable+Info.plist)을 "Spiral Buddy Black Helper*"로 rename해
...
exports.default = async function afterPack(context) {
  ...
  const productFilename = context.packager.appInfo.productFilename; // "Spiral Buddy Black"
  ...
  for (const sfx of suffixes) {
    const oldName = `${productFilename} Helper${sfx}`; // "Spiral Buddy Black Helper (GPU)"
    const newName = `${bundleName} Helper${sfx}`; //      "Spiral Buddy Black Helper (GPU)"
