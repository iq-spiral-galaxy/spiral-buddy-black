...
const [
  ...
  readFile(new URL("../client/black-brand.css", import.meta.url), "utf8"),
...
describe("client UI contracts", () => {
  ...
  test("Black-only principles loading is bounded, cancellable and retryable", () => {
    const principlesLoader = app.slice(
      app.indexOf("let _principlesData"),
      app.indexOf("// 원리의 6층 종합 레포"),
    );
    assert.match(principlesLoader, /let _principlesRequestController = null/);
    assert.match(principlesLoader, /let _principlesRequestEpoch = 0/);
    assert.match(
      principlesLoader,
      /fetchJson\(\"\/api\/principles\", \{[\s\S]*?signal: controller\.signal,[\s\S]*?timeoutMs: 15_000,[\s\S]*?retries: 1/,
    );
    assert.match(principlesLoader, /_principlesRequestController\?\.abort\(\)/);
    assert.match(principlesLoader, /class="activity-retry principles-retry"/);
    assert.doesNotMatch(principlesLoader, /fetch\(\"\/api\/principles\"\)/);
  });

  test("Black identity fallbacks and release assets stay edition-safe", () => {
    assert.doesNotMatch(app, /#2459d3/);
    assert.match(app, /p\.color \?\? "var\(--blue-cobalt\)"/);
    assert.match(app, /d\.color \?\? "var\(--blue-cobalt\)"/);
    assert.doesNotMatch(app, /vault의 spiral-buddy\/\.trash\//);
    assert.match(app, /이 워크스페이스의 휴지통으로 이동/);
    assert.match(app, /w\.vaultSubDir \?\? "spiral-buddy-black"/);
    assert.match(app, /ws\.vaultSubDir \?\? "spiral-buddy-black"/);
    assert.match(releaseWorkflow, /run: pnpm build/);
    assert.doesNotMatch(releaseWorkflow, /run: pnpm exec tsc/);
    assert.match(
      releaseWorkflow,
      /Spiral-Buddy-Black-latest-arm64\.dmg/,
    );
    assert.match(
      releaseWorkflow,
      /Spiral-Buddy-Black-latest-Setup\.exe/,
    );
    assert.match(pnpmWorkspace, /electron: true/);
  ...
  test("light composer has clear input and action surfaces with rounded desktop junctions", () => {
    ...
    assert.match(html, /black-brand\.css\?v=0\.6\.11/);
  ...
  test("all message surfaces share safe progressive Markdown and raw-source copy", () => {
    ...
    assert.match(app, /\brenderMarkdown\b/);
