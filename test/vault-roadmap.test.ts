...
describe("roadmap: discoverRoadmaps + loadRoadmapChapters fixture", () => {
  ...
  test("loadRoadmapChapters: title from frontmatter > first H1 > basename (black: no chapter-prefix strip at this layer)", async () => {
    const { root } = await buildRoadmapRepo();
    invalidateRoadmapCaches();
    const [rm] = await discoverRoadmaps(root);
    const chapters = await loadRoadmapChapters(rm!);
    const byId = new Map(chapters.map((c) => [c.id, c]));
    // ADAPTED (per-color divergence): Blue strips a "Chapter N" prefix inside
    // loadChapterFile via stripChapterPrefix(). Black instead uses
    // stripTitleEmoji() here and defers Chapter/Ch prefix removal to the
    // client display layer (v0.1.x, cf. Blue v0.5.108/109). So at the
    // chapter-load layer black keeps the fm title verbatim.
    // 02 fm title "Chapter 2 Bar Title" — kept as-is (prefix NOT stripped here).
    assert.equal(byId.get("02-bar.md")!.title, "Chapter 2 Bar Title");
  ...
  // ADAPTED (per-color divergence): Blue's loadChapterFile runs
  // stripChapterPrefix() so "# Chapter 1 - Intro" → "Intro". Black's
  // loadChapterFile runs stripTitleEmoji() only — Chapter/Ch prefix removal is
  // deferred to the client display layer (displayRepoName in client/app.js).
  // So at the chapter-load layer black keeps the H1 verbatim. This test pins
  // black's ACTUAL behavior (titles untouched here, "Chrome" still kept).
  test("chapter title at load layer: H1 kept verbatim (black strips emoji, NOT Chapter prefix)", async () => {
    ...
    assert.equal(byId.get("a.md"), "Chapter 1 - Intro");
    assert.equal(byId.get("b.md"), "Ch2: Deep Dive");
