...
import { boundaryStatus, suggestNext } from "./spiral.js";
  ...
  app.get("/chapters", async (c) => {
    ...
    return c.json({
      ...
      chapters: chapters.map((ch) => {
        ...
        const latestNote = matchingNotes.reduce(
          (latest, note) =>
            !latest || note.modifiedAt > latest.modifiedAt ? note : latest,
          matchingNotes[0],
        );
        ...
        return {
          ...
          noteLinks,
          lastDate,
          // Black Physis: 최근 노트가 원리의 경계·극한을 충분히
          // 검증하지 못했으면 허브가 다음 챕터보다 같은 챕터의 더 깊은
          // 나선을 먼저 제안한다. 추가 LLM 요청 없이 즉시 계산한다.
          needsBoundaryReview: latestNote
            ? boundaryStatus(latestNote.body) === "얇음"
            : false,
...
export function createApi(config: Config, deps: { client?: ClaudeClient } = {}) {
  ...
  // 3-b. d1 완료 챕터 검증 (카드/시도는 notes와 별도 JSON)
