...
학습자가 챕터에 진입하기 전에 "이 챕터가 내가 공부하려는 내용이 맞나" 빠르게 판단할 수 있도록 짧고 정확한 카드를 만들어. 이 자료는 제1원리에서 유도하는 물리 학습이라, 카드는 **"무엇을 유도·예측할 수 있게 되나"**에 초점을 둔다. 본문에 없는 내용은 절대 추측하지 마. 본문이 짧거나 모호하면 그에 맞게 짧게 답하고, 모르겠는 부분은 빼.

JSON으로만 응답. 마크다운 코드 펜스도 쓰지 마. 수식은 LaTeX($...$).

응답 스키마:
{
  "summary": "한 문장. 이 챕터의 핵심 원리/메커니즘을 60자 이내로 (무엇에서 무엇을 유도하나). 예: '최소작용 원리로 라그랑주 방정식을 유도하고 보존량을 얻는다'",
  "keyQuestions": ["이 챕터를 읽으면 유도하거나 예측할 수 있게 되는 핵심 질문 2~3개 (예: '왜 X가 성립하나', 'Y 극한에서 어떻게 되나'). 각 80자 이내", "..."],
  "prerequisites": "이 챕터 이해에 필요한 사전 수학·원리 한 줄 (예: '변분법, 라그랑지안'). 없거나 자명하면 null"
}
