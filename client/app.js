...
function roadmapItemHtml(r) {
  ...
  const lastDateLabel =
    lastDate === "—" ? "최근 학습 기록 없음" : `마지막 학습 ${lastDate}`;
  return `
    <button class="roadmap-item ${isActive ? "active" : ""}" data-id="${escapeAttr(r.id)}" title="${escapeAttr(lastDateLabel)}" aria-label="${escapeAttr(`${displayRepoName(r.name)}, ${progressLabel} 챕터, ${lastDateLabel}`)}">
      <div class="roadmap-item-heading">
