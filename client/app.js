...
function wireSidePanels() {
  ...
  if (els.principlesSearch) {
    els.principlesSearch.addEventListener("input", (e) => {
      _principlesQuery = e.target.value;
      if (_principlesData) renderPrinciples(_principlesData, _principlesQuery);
    });
  }
  ...
  const chaptersTask = (async () => {
    try {
      ...
      state.chapters = state.chapters.map((chapter) => ({
        ...chapter,
        title: stripEmoji(chapter.title),
      }));
      renderChapters();
...
async function refreshUpdateBanner({ force = false } = {}) {
  ...
  // releases link 기본값
  const RELEASES_URL =
    "https://github.com/iq-spiral-galaxy/spiral-buddy-black/releases/latest";
  if (releasesLink) {
      ...
      return `
        <div class="ws-row ${isActive ? "active" : ""}">
          <div class="ws-row-main">
            ...
            <div class="ws-row-vaultsub">노트: <code>vault/${escapeHtml(w.vaultSubDir ?? "spiral-buddy-black")}/</code></div>
          </div>
...
function openRemoveWorkspaceModal(ws) {
  ...
  document.getElementById("remove-ws-notes-path").innerHTML = `<code>${escapeHtml(_settingsCache.vaultPath ?? "")}/${escapeHtml(ws.vaultSubDir ?? "spiral-buddy-black")}</code> 이 옆 폴더로 이동(보관)됩니다.`;
...
function renderCuratedPresets() {
  ...
  grid.innerHTML = presets
    .map((p) => {
      ...
      return `
        <button class="curated-preset-card${p.recommended ? " recommended" : ""}${p.heavy ? " heavy" : ""}" data-preset="${escapeAttr(p.id)}" type="button" style="--preset-tone: ${escapeAttr(p.color ?? "var(--blue-cobalt)")}" ${!_curatedState.parentDir || _curatedState.busy ? "disabled" : ""}>
    ...
    .map((d) => {
      ...
      const btnLabel = isAllDone
        ...
        : isPartial
          ...
          : `받기 (${repos.length})`;
      return `
        <div class="curated-domain-row ${isAllDone ? "all-done" : ""} ${isPartial ? "partial" : ""}" style="--domain-tone: ${escapeAttr(d.color ?? "var(--blue-cobalt)")}">
...
// → 실제 구현은 ./stream.js로 분리 (refactor(5)).
// ──────────────────────────────────────────────────────────
...
let _principlesQuery = "";
let _principlesCountController = null;
let _principlesCountEpoch = 0;
let _principlesRequestController = null;
let _principlesRequestEpoch = 0;
const PRINCIPLE_ICON_BY_KEY = {
  symmetry: "atom",
  "least-action": "orbit",
  entropy: "thermo",
...
async function refreshPrinciplesCount() {
  ...
  _principlesCountController?.abort();
  const controller = new AbortController();
  _principlesCountController = controller;
  const epoch = ++_principlesCountEpoch;
  const isCurrent = () =>
    epoch === _principlesCountEpoch &&
    _principlesCountController === controller &&
    !controller.signal.aborted;
  try {
    ...
    if (_principlesCountController === controller) {
      _principlesCountController = null;
    }
  }
}
...
async function openPrinciplesModal() {
  ...
  _principlesRequestController?.abort();
  const controller = new AbortController();
  _principlesRequestController = controller;
  const epoch = ++_principlesRequestEpoch;
  const isCurrent = () =>
    epoch === _principlesRequestEpoch &&
    _principlesRequestController === controller &&
    !controller.signal.aborted &&
    !els.principlesModal.classList.contains("hidden");
  ...
  els.principlesList.innerHTML = `<div class="principles-empty principles-load-state"><span class="activity-loading"><span class="inline-spinner" aria-hidden="true"></span>횡단 원리를 불러오는 중…</span></div>`;
  ...
  try {
    ...
    els.principlesSearch?.focus();
  } catch (err) {
    if (isAbortedRequest(err, controller.signal) || !isCurrent()) return;
    const reason = readableLoadError(err);
    els.principlesList.innerHTML = `
      <div class="principles-empty principles-load-state">
        <span>횡단 원리를 불러오지 못했어요 · ${escapeHtml(reason)}</span>
        <button class="activity-retry principles-retry" type="button">다시 시도</button>
      </div>
    `;
    els.principlesList
      .querySelector(".principles-retry")
      ?.addEventListener("click", openPrinciplesModal);
  } finally {
    if (_principlesRequestController === controller) {
      _principlesRequestController = null;
    }
  }
}
...
// 원리의 6층 종합 레포(synthRepo)에 해당하는 *설치된* 로드맵 찾기 (없으면 null).
function _findSynthRoadmap(synthRepo) {
  if (!synthRepo) return null;
  const t = String(synthRepo).toLowerCase();
  return (
    (state.roadmaps ?? []).find((r) => {
      const repo = String(r.hierarchy?.repo ?? r.repo ?? "").toLowerCase();
      const id = String(r.id ?? "").toLowerCase();
      return repo === t || id === t || id.split("/")[0] === t;
    }) ?? null
  );
}
...
function renderPrinciples(data, query) {
  ...
  // 검색 필터 — 노트 단위(주제·레이어·메타·창발 스니펫·원리명)
  let shownNotes = 0;
  const groups = [];
  for (const p of principles) {
    const notes = (p.notes ?? []).filter((n) => {
      if (!q) return true;
      const hay =
        `${n.topic ?? ""} ${n.chapter ?? ""} ${n.repo ?? ""} ${n.roadmapName ?? ""} ${n.domain?.name ?? ""} ${n.emergence ?? ""} ${p.label ?? ""}`.toLowerCase();
      return hay.includes(q);
    });
    if (!notes.length) continue;
    shownNotes += notes.length;
    groups.push({ p, notes });
  }
  ...
  if (!groups.length) {
    els.principlesList.innerHTML = q
      ? `<div class="principles-empty">"${escapeHtml(query)}"에 매칭되는 노트가 없어요.</div>`
      : `<div class="principles-empty">아직 모인 횡단 원리가 없어요.<br/>세션을 끝내고 노트를 저장하면, 노트에 달린 원리 태그(대칭·최소작용·엔트로피·정보·창발)가 여기에 레이어를 가로질러 쌓여요.</div>`;
    return;
  }

  const html = [];
  for (const { p, notes } of groups) {
    // 6층 종합 레포로의 다리 — 설치돼 있으면 전환 버튼, 아니면 미설치 안내.
    const synth = _findSynthRoadmap(p.synthRepo);
    const bridge = synth
      ? `<button class="principle-synth" data-synth-id="${escapeAttr(synth.id)}" title="${escapeAttr(displayRepoName(p.synthRepo))} 로드맵으로 이동">이 원리로 종합 학습 →</button>`
      : p.synthRepo
        ? `<span class="principle-synth-missing" title="${escapeAttr(displayRepoName(p.synthRepo))} 종합 레포가 아직 설치되지 않았어요 (설정에서 받기)">종합 레포 미설치</span>`
        : "";
    html.push(
      `<div class="principle-group">` +
        `<div class="principle-head">${uiIconHtml(PRINCIPLE_ICON_BY_KEY[p.key] ?? "sparkle", "principle-emoji")}` +
        `<span class="principle-label">${escapeHtml(cleanUiLabel(p.label ?? p.key))}</span>` +
        `<span class="principle-count">${notes.length}</span>${bridge}</div>`,
    );
    for (const n of notes) {
      const color = n.domain?.color ?? "var(--accent)";
      const layer = n.domain
        ? displayRepoName(n.domain.name)
        : n.repo
          ? displayRepoName(n.repo)
          : (n.roadmapName ?? "");
      ...
      const viaBadge =
        n.via === "mention"
          ...
          : "";
      const snippet = n.emergence
        ? `<span class="principle-note-snippet">${escapeHtml(n.emergence)}</span>`
        : "";
      const inner =
  ...
  els.principlesList
    .querySelectorAll(".principle-note[data-obsidian]")
    .forEach((btn) => {
      btn.addEventListener("click", () =>
  ...
  els.principlesList
    .querySelectorAll(".principle-synth[data-synth-id]")
    .forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        e.stopPropagation();
        closePrinciplesModal();
        await switchRoadmap(btn.dataset.synthId);
      });
    });
}
...
function openDeletePopover(anchorEl, target) {
  ...
  const hint = `<div class="delete-popover-hint">이 워크스페이스의 휴지통으로 이동 — 복구 가능</div>`;
...
const QUIZ_LEVELS = [
  {
    ...
    tone: "recall",
    prompt:
      "지금까지 다룬 내용의 핵심 원리를, 제1원리에서 짚는 짧은 질문 2개를 내줘. 답은 알려주지 말고, 내가 먼저 유도하거나 설명해보게 해줘.",
  },
  {
    level: 2,
    label: "극한·적용",
    tone: "apply",
    prompt:
      "오늘 배운 원리를 다른 상황이나 극한(예: v→c, T→0, N→∞, ℏ→0)에 적용해야 답할 수 있는 질문 2개를 내줘. 답은 알려주지 마.",
  },
  {
    level: 3,
    label: "직관의 함정",
    tone: "challenge",
    prompt:
      "오늘 다룬 원리에 대한 흔한 직관의 함정, 또는 가정이 깨지는 경계(언제 무너지나)를 찌르는 날카로운 질문 2개를 내줘. 답은 알려주지 마.",
  },
  {
    level: 4,
    label: "횡단·예측",
    tone: "synthesis",
    prompt:
      "오늘 다룬 원리 + 사전 지식이나 다른 레이어의 횡단 원리(대칭·최소작용·엔트로피·정보·창발)까지 엮어, 반증 가능한 예측 하나를 끌어내야 하는 종합 문제 1개를 내줘. 풀어주지 말고 내가 생각해보게 해줘.",
  },
];
